...
import { createEmptySession, estimateCalories, formatPace, formatTime, haversineKm, type GpsPoint, type RunSessionData } from './runState';
...
import { startEngine, stopEngine, onLocationUpdate, cleanup as cleanupBridge, type GpsLocation } from '../../services/native/GpsBridge';
...
export default function RunSession({ onBackHome = () => undefined, onViewMap = () => undefined }: Props) {
  ...
  // Phase 1.6.1B: GpsBridge cleanup handle
  ...
  useEffect(() => {
    ...
    isNativeRef.current = typeof window !== 'undefined' && !!window.Capacitor?.isPluginAvailable('GpsPlugin');
  }, []);

  const stopGps = useCallback(() => {
    if (gpsCleanupRef.current) { gpsCleanupRef.current(); gpsCleanupRef.current = null; }
    cleanupBridge();
    stopEngine().catch(() => {});
  }, []);
  ...
  const toGpsPoint = (loc: GpsLocation): GpsPoint => ({
  ...
  const startGps = useCallback(() => {
    if (gpsCleanupRef.current !== null) return;
    // Phase 1.6.1B: Use GpsBridge (native first, Web fallback inside)
    startEngine().then(() => {
      const unsub = onLocationUpdate((loc) => {
        setSession((previous) => {
          const points = [...previous.points, toGpsPoint(loc)];
          let distanceKm = previous.distanceKm;
          if (points.length >= 2) {
            const prior = points[points.length - 2];
            distanceKm += haversineKm(prior.latitude, prior.longitude, loc.latitude, loc.longitude);
          }
    ...
    <div className="run-session-v1">
      ...
      {mode === 'manual' ? <ManualRunEntry onSave={(input) => persistRun(input)} saving={saving} /> : (
        <section className="gps-run-card">
          {gpsGranted !== true ? <div className="gps-permission-card"><div className="gps-permission-icon"><E23Icon name="run" size={30} /></div><h2>{gpsGranted === false ? '定位权限尚未开启' : '使用GPS记录真实轨迹'}</h2><p>授权后会在本机记录距离、时长和轨迹。GPS能力仍需真机验证。</p><button className="primary-action" onClick={async () => setGpsGranted(await requestGpsPermission())}>{gpsGranted === false ? '重新请求定位' : '允许定位并继续'}</button></div> : <>
