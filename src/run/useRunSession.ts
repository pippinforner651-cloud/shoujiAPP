...
export function useRunSession(client: RunClient = nativeRunClient) {
  ...
  const finish = useCallback(async () => {
    const activityId = stateRef.current.activityId;
    await client.stopRun();
    if (activityId) {
      const points = await client.loadFullTrack(activityId);
