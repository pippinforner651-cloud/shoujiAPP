...
export default function ProfilePage() {
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-50 pb-6">
      ...
      {/* 跑步记录 */}
      <Section title="我的跑步记录（本机保存，关闭应用不丢失）">
          ...
          {store.records.slice(0, 10).map((r) => (
            <div key={r.id} className="flex items-center justify-between bg-slate-50 rounded-xl px-3.5 py-2.5">
              <div>
                ...
                <div className="text-xs text-slate-500">{new Date(r.ts).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })} · {{ gps: 'GPS', sim: '演示', manual: '手动补录', import: '导入' }[r.source]}</div>
              </div>
        ...
        <div className="space-y-2">
          {INTEGRATIONS.map((it) => (
            <button key={it.key} onClick={() => setShowInt(showInt === it.key ? null : it.key)}
              ...
              <span className="text-xs px-2 py-1 rounded-full bg-slate-200 text-slate-500">未接入</span>
            </button>
          ))}
      ...
      <div className="px-4 mt-4">
        ...
        <div className="text-center text-xs text-slate-400 mt-4">E23跑起来 · {CONFIG.APP_EDITION} · 健康到永远<br />Commit {CONFIG.COMMIT_SHA}</div>
      </div>
