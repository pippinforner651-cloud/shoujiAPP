...
export default function RunPage() {
  ...
  const prepare = async () => {
    ...
    try {
      ...
      if (!readiness.ready) setMessage(readiness.gpsEnabled ? '请允许精确位置权限' : '请打开手机 GPS 定位开关');
  ...
  return <div className="min-h-full bg-slate-950 text-white pb-24">
    ...
    <main className="px-4 space-y-4">
      {mode === 'outdoor' && <>
        ...
        {view === 'map' && (state.currentPoint || active || state.phase === 'waiting_gps' || state.phase === 'recovery' || state.phase === 'done') && <RunMap mode={state.phase === 'done' ? 'post' : active ? 'running' : 'pre'} currentPoint={state.currentPoint} track={state.geoTrail} accuracyM={state.currentPoint?.accuracyM ?? null} follow={follow} onFollowChange={setFollow} onRenderedPointCount={(pointCount) => run.dispatch({ type: 'MAP_RENDERED', pointCount })} />}
        ...
        {state.phase === 'waiting_gps' && <div className="rounded-3xl bg-slate-900 p-5 text-center"><div className="text-lg font-bold">正在获取真实 GPS</div><p className="text-sm text-slate-400 mt-2">此时尚未创建跑步记录，首个合格定位点到达后再开始。</p><div className="grid grid-cols-2 gap-2 mt-4"><button className="py-3 rounded-full bg-slate-700" onClick={run.cancelPreparation}>取消</button><button className="py-3 rounded-full bg-orange-500 font-bold" onClick={startOutdoor}>确认开始</button></div></div>}
        {active && <><Metrics distanceM={distanceM} durationSec={durationSec} pace={pace} kcal={kcal} /><div className="grid grid-cols-2 gap-3"><button className="py-4 rounded-full bg-slate-700 font-bold" onClick={state.phase === 'paused' ? run.resume : run.pause}>{state.phase === 'paused' ? '继续' : '暂停'}</button><button className="py-4 rounded-full bg-orange-500 font-bold" onClick={finishOutdoor}>结束保存</button></div></>}
